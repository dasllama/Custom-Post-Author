...
function cpa_init() {
	...
	add_filter( 'the_author', 'cpa_the_author' );
...
function cpa_the_author( $display_name ) {
	...
	// Check URL
	$author_url = get_post_meta( $post->ID, 'cpa_author_url', TRUE );
	if ( '' != $author_url )
		$return_name = '<a href="' . $author_url . '">' . $return_name . '</a>';

	// register filter for other plugins
	$return_name = apply_filters( 'cpa_the_author', $return_name, $author_name, $author_url );
...
function cpa_meta_box() {
	...
	<table class="form-table">
		...
		<tr>
			<th><label for="cpa_author_url"><?php _e( 'Author URL', CPA_TEXTDOMAIN ); ?></label></th>
